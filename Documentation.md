...
Therefore, all messages are encrypted using AES 128-bit CBC and HMAC is calculated (with SHA-256) to ensure authenticity and integrity of the message. HMAC uses 128-bit keys. This encryption method is itself secure, so messages can be sent also using HTTP. The format of HTTP message body is <message_encrypted>.<hmac_of_the_message>

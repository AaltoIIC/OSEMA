...
/*Function comparison */
function comm_comparison(sort_by){
    return function(a,b){
...
$(document).ready(function(){
    $("#communication_order_inverse").on("change", function(){
...
$(document).ready(function(){
    $("#communication_order_option").on("change", function(){
        var elements = $(".table > tbody >tr").get();
        var rule = $("#communication_order_option").val();
        elements.sort(comm_comparison(rule));
        if (document.getElementById("communication_order_inverse").checked) {
          elements.reverse();
        }
        var elem = $(".table > tbody").get()[0];
        while (elem.firstChild){
...
$(document).ready(function(){
    $("#communication_keyword").on("change", function(){
      var elements = $(".table > tbody > tr").get();
      var string = $("#communication_keyword").val().toLowerCase();
      for (i = 0, len=elements.length; i<len; i++){
...
$(document).ready(function(){
    ...
    for (var i = 0; i < delete_buttons.length; i++) {
      delete_buttons[i].onclick = function() {
        selection = window.confirm('Are you sure? All sensors, which utilize this communication technology instance are also deleted.');
        if (selection) {
